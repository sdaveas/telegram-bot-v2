...
class Bot:
    def __init__(
        self, token: str, db_path: str = "database/messages.db", translate_api_url: str = ""
    ):
        ...
        self.application.add_handler(
            TGMessageHandler(filters.PHOTO, PhotoHandler(self), block=False)
        )
        self.application.add_handler(MessageReactionHandler(ReactionHandler(self)))
        self.application.add_handler(
            TGMessageHandler(filters.REPLY, ReplyHandler(self), block=False)
        )
        self.application.add_handler(
            TGMessageHandler(
                filters.TEXT & ~filters.COMMAND & ~filters.REPLY, TextHandler(self), block=False
            )
        )
        self.application.add_handler(
            TGMessageHandler(filters.VOICE, VoiceMessageHandler(self), block=False)
        )
